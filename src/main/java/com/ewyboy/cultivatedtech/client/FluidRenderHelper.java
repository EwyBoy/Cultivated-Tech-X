...
public class FluidRenderHelper {
     ...
     * @param w     Width. 1 = full X-Width
     * @param h     Height. 1 = full Y-Height
     * @param sN     Disable render side North
     * @param sS     Disable render side South
     * @param sE     Disable render side East
     * @param sW     Disable render side West
     * @param sU     Disable render side Up
     * @param sD     Disable render side Down
     */
    public static void renderSidedFluidCuboid(FluidStack fluid, BlockPos pos, double x, double y, double z, double w, double h, double d, boolean sN, boolean sS, boolean sE, boolean sW, boolean sU, boolean sD) {
        double wd = (1d - w) / 2d;
        double hd = (1d - h) / 2d;
        double dd = (1d - d) / 2d;

        renderSidedFluidCuboid(fluid, pos, x, y, z, wd, hd, dd, 1d - wd, 1d - hd, 1d - dd, sN, sS, sE, sW, sU, sD);
    }
     ...
     * Renders block with offset x/y/z from x1/y1/z1 to x2/y2/z2 inside the block local coordinates, so from 0-1
     */
    public static void renderSidedFluidCuboid(FluidStack fluid, BlockPos pos, double x, double y, double z, double x1, double y1, double z1, double x2, double y2, double z2, int color, boolean sideNorth, boolean sideSouth, boolean sideEast, boolean sideWest, boolean sideUp, boolean sideDown) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder renderer = tessellator.getBuffer();
        renderer.begin(GL11.GL_QUADS, DefaultVertexFormats.BLOCK);
        mc.textureManager.bindTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE);
